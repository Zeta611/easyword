    ...
    ? IfAny<
        U,
        {
          public: {
            ...
            Views: Record<string, any>; // eslint-disable-line @typescript-eslint/no-explicit-any
            Functions: Record<string, any>; // eslint-disable-line @typescript-eslint/no-explicit-any
          };
        },
        U
      >
  ...
  const fetchPage = async (skip: number) => {
    ...
    if (error) {
      ...
      const deduplicatedData = ((newData || []) as TData[]).filter(
        // @ts-expect-error Works on any data
